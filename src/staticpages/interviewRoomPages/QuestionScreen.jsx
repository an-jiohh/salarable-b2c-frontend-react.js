...
const QuestionScreen = ({ onNext }) => {
  ...
  const answerList = {
    ...
    fe: [
      "사내에 도입하는 과정에서 가장 큰 도전 과제는 기존의 상태관리 라이브러리와의 호환성 문제를 해결한 것이었습니다. 탠스텍 쿼리를 기반으로 전환하는 데 있어 코드 구조와 데이터 흐름을 재설계 해야 했습니다. 도입의 초기 단계에서는 프로젝트에 비해핵심적인 에이피아이 요청 몇 가지를 대상으로 탠스텍 쿼리를 테스트하여 문제점을 사전에 식별했습니다. 이를 통해 기존 로직을 점진적으로 텐스택 커리으로 대체해 가면서 안정적인 전환이 가능하도록 진행했습니다",
      "해당 부분은 잘모르겠습니다.",
      "Redux를 사용해본 경험도 없습니다. 해당 이론에 대해 알고있는 부분도 부족합니다.",
      "",
      "",
      "",
      "",
  ...
  const handleRecordButtonClick = () => {
    if (isRecording) {
      ...
      setIsRecording(true);
      setCurrentAnswer(answerList.fe[questionCount]);
          ...
          <div className="flex-grow bg-white dark:bg-gray-800 rounded-bl-2xl rounded-r-2xl border border-gray-200 dark:border-gray-700">
            <div className="p-4">
              <div className="mb-2 text-lg font-semibold text-[#5865F2] dark:text-[#7289da]">
                <div>{currentQuestion.keyword}에 대한 질문입니다.</div>
              </div>
